...
@define-color surface_rgba {{surface_rgba}};

* {
  all: unset;
  font-family: "{{font_family}}", "Fira Sans", sans-serif;
  font-size: 13px;
...
.floating-notifications.background .notification-row
  ...
  box-shadow: 0 8px 32px alpha(@red, 0.12);
  border-radius: 14px;
  margin: 8px 16px 8px 8px;
}
...
.widget-dnd {
  ...
  border-radius: 10px;
  border: 1px solid alpha(@overlay, 0.22);
  color: @fg;
  font-size: 13px;
  font-weight: 600;
}
...
.widget-dnd > switch {
  ...
  min-width: 44px;
  min-height: 24px;
  border: 1px solid alpha(@overlay, 0.3);
}
...
.widget-buttons-grid button.toggle:checked {
  ...
  border: 1px solid alpha(@accent, 0.5);
  border-radius: 14px;
  box-shadow: 0 0 12px alpha(@accent, 0.15);
}
...
.widget-buttons-grid > flowbox > flowboxchild:nth-child(n+5) button label {
  color: transparent;
}
...
.widget-volume {
  ...
  border-radius: 10px;
  background: alpha(@surface, 0.5);
  border: 1px solid alpha(@overlay, 0.2);
  color: @fg;
}
...
.control-center .notification-content .time {
  font-size: 10px;
  color: alpha(@overlay, 0.8);
}
